...
class Theater_Event_Order {
	...
	static function calculate_event_order_index( $event_id ) {
		...
		$event_dates_obj = new Theater_Event_Date_List( array( 'start' => 'now', 'production' => $event_id ) );
		$event_dates = $event_dates_obj->get();
		
		// Use the last event date if no upcoming event dates are found.
		if ( empty( $event_dates ) ) {
			$event_dates_obj = new Theater_Event_Date_List( array( 'production' => $event_id ) );
			$event_dates = array_reverse( $event_dates_obj->get() );
		}
		...
		return $event_dates[0]->get('startdatetime');
	}
	...
	static function calculate_event_date_order_index( $event_date_id ) {
		...
		$event_date = new Theater_Event_Date( $event_date_id );
		return $event_date->get('startdatetime');
	...
	static function update_order_index_when_event_date_is_updated( $meta_id, $object_id, $meta_key, $meta_value ) {
		if ( 'event_date' == $meta_key ) {
			...
			self::set_order_index( $object_id );

			$event_date = new Theater_Event_Date( $object_id );
			$event = $event_date->get('event');
