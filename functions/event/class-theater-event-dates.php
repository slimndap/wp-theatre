...
class Theater_Event_Dates {
	...
	static function init() {
		...
		add_action( 'updated_post_meta', 'Theater_Event_Dates::sync_season', 20 ,4 );
		add_action( 'added_post_meta', 'Theater_Event_Dates::sync_season', 20 ,4 );
		add_action( 'set_object_terms', 'Theater_Event_Dates::sync_categories',20, 6 );

		add_action( 'before_delete_post','Theater_Event_Dates::delete_dates' );
		add_action( 'wp_trash_post','Theater_Event_Dates::trash_dates' );
		add_action( 'untrash_post','Theater_Event_Dates::untrash_dates' );
	...
	static function sync_season( $meta_id, $object_id, $meta_key, $meta_value ) {
		...
		if ( $meta_key == WPT_Season::post_type_name ) {
			...
			if ( $post->post_type == WPT_Production::post_type_name ) {
				...
				// avoid loops
				remove_action( 'updated_post_meta', 'Theater_Event_Dates::sync_season', 20 ,4 );
				remove_action( 'added_post_meta', 'Theater_Event_Dates::sync_season', 20 ,4 );
